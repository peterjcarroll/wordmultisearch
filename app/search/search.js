...
.controller('SearchPageController', function ($scope, $sce) {
    ...
    var getRhinoSpikeLink = function(lang, search) {
      var langCode = {'eng': '1', 'epo': '22', 'spa': '2'};
    ...
    var getForvoLink = function(lang, search) {
      var langCode = {'eng': 'en', 'epo': 'eo', 'spa': 'es'};
    ...
    var getTatoebaLink = function(lang, search) {
      var url = 'http://tatoeba.org/{lang}/sentences/search?query={search}&from={lang}&to=und'.replace('{search}', encodeURIComponent(search));
      url=url.replace(/\{lang\}/g, lang);
      return $sce.trustAsResourceUrl(url);
    };

    var getLingueeLink = function(lang, search) {
      var langCode = {'spa': 'english-spanish'};
    ...
    var isSupported = function(tab, lang){
      var langSupport = {
        'rhinospike': 'eng epo spa',
        'forvo': 'eng epo spa',
        'tatoeba': 'eng epo spa',
        'linguee': 'spa',
        'googleimages': 'eng spa',
        'googletranslate': 'epo spa',
        'transdict': 'epo spa',
        'monodict': 'eng epo spa'
      };
